...
from emmental.modules import ThresholdBinarizer, TopKBinarizer, MagnitudeBinarizer
    ...
    for name, param in st.items():
        ...
        if "mask_scores" in name:
            ...
            if pruning_method != "magnitude":
                ...
                csv_list.append([name,str(round(100 * mask_ones / param.numel(), 3)),str(mask_ones)])
        else:
            if pruning_method == "magnitude" and "bias" not in name and "LayerNorm" not in name:
                mask_ones = MagnitudeBinarizer.apply(param, threshold).sum().item()
                print(name.ljust(60, " "), str(round(100 * mask_ones / param.numel(), 3)).ljust(20, " "), str(mask_ones))
                remaining_count += mask_ones
                csv_list.append([name,str(round(100 * mask_ones / param.numel(), 3)),str(mask_ones)])
        ...
        # Extract layers
        #if name == "bert.encoder.layer.0.attention.self.value.weight":
        #    save_name = name.replace(".","_"); save_name += ".csv"
        #    np.savetxt(os.path.join(serialization_dir,save_name), param.numpy(), delimiter=",")

    print("")
    print("Remaining Weights (global) %: ", 100 * remaining_count / encoder_count)
    df = pd.DataFrame(csv_list,index=None)
    df.to_csv(os.path.join(serialization_dir,"parameter_count.csv"),index=None,header=None)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pruning_method",
        choices=["l0", "topK", "sigmoied_threshold","magnitude"],
        type=str,
        required=True,
